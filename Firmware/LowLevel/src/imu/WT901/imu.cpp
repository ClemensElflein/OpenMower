...
bool imu_read(float *acceleration_mss, float *gyro_rads, float *mag_uT)
{
    ...
    // gyro_rads[0] = (float)IMU.stcGyro.w[0] / 32768 * 2000;
    // gyro_rads[1] = (float)IMU.stcGyro.w[1] / 32768 * 2000;
    // gyro_rads[2] = (float)IMU.stcGyro.w[2] / 32768 * 2000;

    gyro_rads[0] = (float)IMU.stcAngle.Angle[0]/32768.0f*180.0f;
    gyro_rads[1] = (float)IMU.stcAngle.Angle[1]/32768.0f*180.0f;
    gyro_rads[2] = (float)IMU.stcAngle.Angle[2]/32768.0f*180.0f;
